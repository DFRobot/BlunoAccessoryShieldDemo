...
public class ColorPicker extends View {
	...
	public boolean mIsSwitchOn = false;
	 ...
	 */
	public int mColorWheelThickness;

	/**
	 * The radius of the color wheel.
	 */
	public int mColorWheelRadius;
	public int mPreferredColorWheelRadius;
	 ...
	 * color.
	 */
	private Paint mCenterCircleTextPaint;
...
//		mColorWheelPaint.setStrokeWidth(mColorWheelThickness*3/2);
		...
		mCenterCirclePaint.setColor(0xff707070);//0xff4dd5b5
	...
	protected void onDraw(Canvas canvas) {
		...
		// top.
		canvas.drawCircle(pointerPosition[0], pointerPosition[1],
				mColorPointerRadius*4/5, mPointerColor);

		// Draw the halo of the center colors.
		canvas.drawCircle(0, 0, mColorCenterHaloRadius, mCenterCirclePaint);

		
		if(mIsSwitchOn){
			canvas.drawText("ON", 0, mColorCenterRadius/4, mCenterCircleTextPaint);
		}
		else{
			canvas.drawText("OFF", 0, mColorCenterRadius/4, mCenterCircleTextPaint);	
		}
...
//		canvas.drawArc(mCenterRectangle, 270, 180, true, mCenterNewPaint);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			float[] pointerPosition = calculatePointerPosition(mAngle);
			if (x >= (pointerPosition[0] - mColorPointerHaloRadius*3)
					&& x <= (pointerPosition[0] + mColorPointerHaloRadius*3)
					&& y >= (pointerPosition[1] - mColorPointerHaloRadius*3)
					&& y <= (pointerPosition[1] + mColorPointerHaloRadius*3)) {
...
//				mCenterNewPaint.setColor(getOldCenterColor());
		...
		//Set centerCircle color according to the user touch event
		if(mIsSwitchOn)
			mCenterCirclePaint.setColor(0xff1bdab1); 
		else
